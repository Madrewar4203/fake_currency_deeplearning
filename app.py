...
@app.route('/history')
def history():
    data=retrivequery1("select * from history where username=%s",(useremail,))
    parse=[]
    ...
    return render_template('history.html',parse=parse)
...
def login():
    if request.method == "POST":
        global useremail
        email = request.form.get('emailPoint')
...
def upload():
    if request.method == 'POST':
        ...
        print(prediction)
        query="INSERT INTO history(username,path,results) VALUES (%s,%s,%s)"
        print(useremail)
        values=(useremail,mypath,prediction)
        executionquery(query,values)
        return render_template("upload.html", mypath = mypath, prediction=prediction)
